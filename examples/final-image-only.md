...
> 本案例作为 v0.3.0 Reconstruction 示例：所有新增版本均属于当前复现流程，不冒充原始历史。
> 创作者**仅上传一张最终作品** `final.png`，无原始 Prompt、无草图、无参数。
> Skill 从 `schema/canonical-assets.yaml` 读取资产规范，生成可验证的复现研究、记录、Manifest 与 DOCX。
...
| `generation_v1` | 文件名由机器 Schema 读取 | 真实 generation backend | ✅ Generated | `[Reconstructed]` |
| `generation_v2` | 按实际诊断需要动态出现 | 真实 generation backend | ✅ Generated | `[Reconstructed]` |
| `generation_request_v1` | Prompt V1 与输入在调用前落盘 | 动态语义解构生成 | ✅ Generated | `[Reconstructed]` |
| `generation_request_v2` | Prompt V2 与 Prompt Evolution 在第二次调用前落盘 | 基于 V1 实测差异生成 | ✅ Generated | `[Reconstructed]` |
...
- **阶段二：Generation V1**（当前复现流程的完整作品版本）
- **阶段三：Generation V2**（由 V1 实际诊断推动的完整深化版本；仅在需要时出现）
- **后期说明**：未取得后期流程证据，不对是否存在人工后期作推断。
