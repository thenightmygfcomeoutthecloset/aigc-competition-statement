...
**不允许**：
- 假设赛事要求，声称"符合官方规定"（未获取规则时）
- 找不到规则时完全拒绝生成

**找到规则时**：进入 Competition-specific mode，依据规则生成。

**未找到规则时**：进入 Generic Draft Mode：
1. 使用通用模板继续执行
2. 在文档顶部加入警示："⚠️ 本文档基于通用模板生成，尚未依据官方赛事要求校验。"
3. 交付时告知用户需对照官方要求修改

**输出**：赛事要求摘要（或 Generic Draft Mode 标注），供后续所有阶段参考
