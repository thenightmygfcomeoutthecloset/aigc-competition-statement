...
机器可读的 [`schema/canonical-assets.yaml`](../schema/canonical-assets.yaml) 是唯一权威资产定义。需要知道资产 ID、文件名、类型、是否必需或 DOCX 嵌入顺序时读取该文件，不在本文复制清单。
...
6. 构建严格 Manifest、装配 DOCX 并完成文件与渲染验证。

任何门禁失败都必须非零退出，不得以缺图、空记录或悬挂路径交付。

## 前期视觉设计

`scripts/reconstruct_assets.py` 使用 OpenCV 与 Pillow 生成前期视觉设计：

- 它只生成 sketch、lineart、color block，不产生任何 `generation_vN`；
